...
## 🔄 사이클 시작 전 필수 룰 (CRITICAL — v0.2.5)

**매 사이클 시작 시 다음 3가지 의무**:

1. **Chrome 완전 재시작** — 모든 탭 닫기 → Chrome 종료 → 재실행
   - 이유: vm 인스턴스 fragmentation, 토큰 만료, 메모리 압박, 60초 락 누적 모두 초기화
   - 1시간+ 누적 시 자동화 실패율 급증
2. **새 채팅 세션 시작** (Cowork)
   - 이유: 이전 사이클 컨텍스트 누적이 vm 추적 혼란 야기
3. **사방넷 자동 로그인 확인** (Chrome 재시작 직후)
   - 토큰 fresh 상태에서 사방넷·풀필먼트 모두 정상 진입

→ Fresh state에서 시작해야 모든 자동화 단계 안정 작동.
→ Step 4 사방넷 다운로드 자동화도 Fresh 상태라면 vm sbForm 정상 채워질 가능성 있음 (v0.2.6 검증 예정).
